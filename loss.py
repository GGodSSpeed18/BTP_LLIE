...
import logging
from utils import  pair_downsampler,calculate_local_variance,LocalMean, FeatureExtractor
from pytorch_msssim import ms_ssim

EPS = 1e-9
PI = 22.0 / 7.0

def ms_ssim_l2_loss(pred, target, alpha=0.70, data_range=1.0, lossfunctionObj=None):
    # l1_loss = torch.nn.functional.l1_loss(pred, target)
    ms_ssim_loss = 1 - ms_ssim(pred, target, data_range=data_range, size_average=True)
    l2_loss = lossfunctionObj._l2_loss(pred, target) * 1000
    return ms_ssim_loss * alpha + (1 - alpha) * l2_loss

class LossFunction(nn.Module):
    def __init__(self, max_epochs=400):
    ...
    def forward(self,input,L_pred1,L_pred2,L2,s2,s21,s22,H2,H11,H12,H13,s13,H14,s14,H3,s3,H3_pred,H4_pred,L_pred1_L_pred2_diff,H3_denoised1_H3_denoised2_diff,H2_blur,H3_blur,epoch=0):
        eps = 1e-9
        input = input + eps
        ...
        # initial_alpha = 0.75
        # final_alpha = 0.25
        # alpha = initial_alpha - (initial_alpha - final_alpha) * (epoch+1 / self.max_epochs)
        # alpha = float(np.clip(alpha, final_alpha, initial_alpha))
        # add01 = ms_ssim_l2_loss(L11, L_pred2, alpha=alpha, data_range=1.0, lossfunctionObj=self)
        # add02 = ms_ssim_l2_loss(L12, L_pred1, alpha=alpha, data_range=1.0, lossfunctionObj=self)
        # loss += add01+add02
